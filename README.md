...
6. [遊戲模式分析 Solo, Duos and Squads](#solo,-duos-and-squads)
...
1. **殺敵數**：每當擊敗一名敵人，代表淘汰掉一名玩家，玩家本身的最終排名就會向前一名，也因此殺敵數與最終排名兩者本身呈現正相關是合理的現象，但不代表高殺敵數一定會換來高排名。
2. **移動距離**：PUBG 獨特的機制 — 毒圈，迫使每位玩家要在時間內朝指定的地點移動，否則生命力會逐漸衰弱最後被淘汰。因次要獲得高排名，就必須不斷的往指定地點移動，因此移動距離也會隨之增加，而移動距離與最終排名自然就會呈現正相關。
